...
function Carousel(options){
	...
	//3.绑定事件
	this.bindEvent();
	//自动播放,有的地方需要自动播放，有的不需要，所以需要判断，如果参数中含有playDuration就自动播放
...
Carousel.prototype.auto = function(){
	...
	//this.oRightArrow.onclick绑定了一个函数，在这里this又是代表创建的对象
...
function tab(){
    var aCity = document.querySelectorAll('.main .job .city-list li');
    var aPost = document.querySelectorAll('.main .job .post ul');
    for(var i = 0;i<aCity.length;i++){
        aCity[i].index = i;
        aCity[i].onclick = function(){
            for(var j = 0;j<aCity.length;j++){
                aCity[j].style.color = 'gray';
                aPost[j].className = 'post-list';
            }
            this.style.color = 'orange';
            aPost[this.index].className = 'post-list post-list-act';
        }
    }
}
function sport(){
    var aBtn = document.querySelectorAll('.main .job .content .post-list-item');
    var lastIndex = 0;
    for(var i = 0;i<aBtn.length;i++){
        (function(i){
            aBtn[i].onclick = function(){
                var heigh = parseInt(getComputedStyle(aBtn[i],false).height);
               if(heigh <= 25){
                    this.className = 'post-list-item current';
               }else{
                    this.className = 'post-list-item';
               }
                lastIndex = i;
            }
        })(i);
